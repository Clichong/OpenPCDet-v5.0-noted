...
def nms_gpu(boxes, scores, thresh, pre_maxsize=None, **kwargs):
    ...
    order = scores.sort(0, descending=True)[1]  # 原本已经排序好
    if pre_maxsize is not None:
        order = order[:pre_maxsize]

    boxes = boxes[order].contiguous()   # (145, 7)
    keep = torch.LongTensor(boxes.size(0))  # (145)
    num_out = iou3d_nms_cuda.nms_gpu(boxes, keep, thresh)   # 核心的nms函数,返回保留下来的个数
