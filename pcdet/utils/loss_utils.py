...
class SigmoidFocalClassificationLoss(nn.Module):
    ...
    def forward(self, input: torch.Tensor, target: torch.Tensor, weights: torch.Tensor):
        ...
        alpha_weight = target * self.alpha + (1 - target) * (1 - self.alpha)    # 平衡因子 (16, 321408, 3)
        ...
        bce_loss = self.sigmoid_cross_entropy_with_logits(input, target)    # 用了另外一种形式实现 (16, 321408, 3)
...
class WeightedSmoothL1Loss(nn.Module):
    ...
    def forward(self, input: torch.Tensor, target: torch.Tensor, weights: torch.Tensor = None):
        ...
        diff = input - target   # 残差
        # code-wise weighting
        if self.code_weights is not None:
            diff = diff * self.code_weights.view(1, 1, -1)  # 回归信息各部分加权
...
def get_corner_loss_lidar(pred_bbox3d: torch.Tensor, gt_bbox3d: torch.Tensor):
    ...
    # 利用box信息构造成8个角点的位置坐标
    pred_box_corners = box_utils.boxes_to_corners_3d(pred_bbox3d)   # (N, 8, 3)
    gt_box_corners = box_utils.boxes_to_corners_3d(gt_bbox3d)   # (N, 8, 3)

    gt_bbox3d_flip = gt_bbox3d.clone()
    gt_bbox3d_flip[:, 6] += np.pi   # 角度+π
    gt_box_corners_flip = box_utils.boxes_to_corners_3d(gt_bbox3d_flip)   # 反转180°的box角点坐标

    # (N, 8)  默认是Frobenius范数定义为矩阵A各项元素的绝对值平方的总和
    corner_dist = torch.min(torch.norm(pred_box_corners - gt_box_corners, dim=2),
                            torch.norm(pred_box_corners - gt_box_corners_flip, dim=2))
    # (N, 8)
    corner_loss = WeightedSmoothL1Loss.smooth_l1_loss(corner_dist, beta=1.0)

    return corner_loss.mean(dim=1)  # 返回的均值
