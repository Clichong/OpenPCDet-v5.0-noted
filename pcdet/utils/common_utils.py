...
def limit_period(val, offset=0.5, period=np.pi):
    val, is_numpy = check_numpy_to_torch(val)   # 格式转换，其中val表示的角度
    ans = val - torch.floor(val / period + offset) * period  # 将方位角限制在[-pi, pi]
    return ans.numpy() if is_numpy else ans

# 功能: 对info中去除name的所有字段信息
def drop_info_with_name(info, name):
    ret_info = {}
    keep_indices = [i for i, x in enumerate(info['name']) if x != name]     # 去除name之后保留的索引
    for key in info.keys():     # 不仅是名字上去除，剩下的其余信息也要去除
        ret_info[key] = info[key][keep_indices]     # 利用索引对其他字段信息进行截取获取
    return ret_info

# 功能: 沿z轴进行随机旋转，这里是对batch的数据进行数据增强，batch内的每帧数据都会分配一个随机选择angle
def rotate_points_along_z(points, angle):
    ...
    rot_matrix = torch.stack((  # 根据旋转角构造沿z轴旋转的旋转矩阵
        cosa,  sina, zeros,
        -sina, cosa, zeros,
        zeros, zeros, ones
    ), dim=1).view(-1, 3, 3).float()
    points_rot = torch.matmul(points[:, :, 0:3], rot_matrix)    # 对点云坐标进行旋转 (B, N, 3)
    points_rot = torch.cat((points_rot, points[:, :, 3:]), dim=-1)      # 将旋转后的点云与原始反射强度特征进行拼接 (B, N, 4)
...
# 作用：设置一个日志控制台输出，另外一个日志在磁盘文件保存
def create_logger(log_file=None, rank=0, log_level=logging.INFO):
    logger = logging.getLogger(__name__)
    logger.setLevel(log_level if rank == 0 else 'ERROR')
    formatter = logging.Formatter('%(asctime)s  %(levelname)5s  %(message)s')
    console = logging.StreamHandler()   # 控制台日志设置
    console.setLevel(log_level if rank == 0 else 'ERROR')
    console.setFormatter(formatter)
    logger.addHandler(console)
    if log_file is not None:            # 磁盘日志设置
...
# 作用：保证每次运行网络的时候相同输入的输出是固定的
def set_random_seed(seed):
    random.seed(seed)
    np.random.seed(seed)
    torch.manual_seed(seed)
    torch.cuda.manual_seed(seed)
    torch.backends.cudnn.deterministic = True   # 默认算法
    torch.backends.cudnn.benchmark = False

# 作用：dataloader中的worker_init_fn参数中使用，是否选择确定随机种子
