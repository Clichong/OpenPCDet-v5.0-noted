...
# 判断点p是否在一个多角点空间区域hull中，返回的每个点是否在的布尔变量
def in_hull(p, hull):
    """
...
def mask_boxes_outside_range_numpy(boxes, limit_range, min_num_corners=1, use_center_to_filter=True):
    ...
    if use_center_to_filter:
        ...
        corners = corners[:, :, 0:2]
        mask = ((corners >= limit_range[0:2]) & (corners <= limit_range[3:5])).all(axis=2)  # (N, 8)
        mask = mask.sum(axis=1) >= min_num_corners  # (N)

    return mask

# 功能: 提出gt的点云，也就是返回背景点，去除前景点
...
def boxes3d_kitti_camera_to_lidar(boxes3d_camera, calib):
    ...
    xyz_lidar = calib.rect_to_lidar(xyz_camera)   # 修正rect坐标系到lidar坐标系
    xyz_lidar[:, 2] += h[:, 0] / 2
...
# 功能: 将gt box扩大范围，也就是增大dx，dy，dz大小
def enlarge_box3d(boxes3d, extra_width=(0, 0, 0)):
    ...
    """
    boxes3d, is_numpy = common_utils.check_numpy_to_torch(boxes3d)  # 转换到tensor上
    large_boxes3d = boxes3d.clone()

    large_boxes3d[:, 3:6] += boxes3d.new_tensor(extra_width)[None, :]   # 扩张范围
