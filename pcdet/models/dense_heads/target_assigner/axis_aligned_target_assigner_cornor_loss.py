...
class AxisAlignedTargetAssignerCornorLoss(object):
    ...
    def assign_targets(self, all_anchors, gt_boxes_with_classes):
        ...
        for k in range(batch_size):
            ...
            if self.use_multihead:  # False 没有启用多尺度构建head
                ...
                target_dict = {
                    ...
                    'reg_cornor_loss': [t['reg_cornor_loss'].view(*feature_map_size, -1) for t in target_list]   # (1，248，216, 2)
                }
                ...
                target_dict['reg_cornor_loss'] = torch.cat(target_dict['reg_cornor_loss'], dim=-1).view(-1)   # 一个点云帧的cornor loss均值

            # 将当前点云场景处理结果分别追加到对应列表中
            bbox_targets.append(target_dict['box_reg_targets'])
            cls_labels.append(target_dict['box_cls_labels'])
            reg_weights.append(target_dict['reg_weights'])
            reg_cornor_loss.append(target_dict['reg_cornor_loss'])
        ...
        reg_cornor_loss = torch.stack(reg_cornor_loss, dim=0)   # (16, 321408) 为了多线程加速

        all_targets_dict = {
            'box_cls_labels': cls_labels,
            'box_reg_targets': bbox_targets,
            'reg_weights': reg_weights,
            'reg_cornor_loss': reg_cornor_loss
        }
    ...
    def assign_targets_single(self, anchors, gt_boxes, gt_classes, matched_threshold=0.6, unmatched_threshold=0.45):
        ...
        reg_cornor_loss = anchors.new_zeros(num_anchors)    # (107136, ) 为了可以多线程加速训练
        if len(gt_boxes) > 0 and anchors.shape[0] > 0:
            ...
            reg_cornor_loss[fg_inds] = loss_utils.get_corner_loss_lidar(fg_anchors, fg_gt_boxes)  # 得到每个gt与anchor的角点损失
        ...
        ret_dict = {
            ...
            'reg_cornor_loss': reg_cornor_loss  # 构建角点损失 (11,)
        }
        return ret_dict
