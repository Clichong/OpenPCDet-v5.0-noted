...
# VEFE全程: Voxel Extra Feature Encoder
class PillarVEFE(VFETemplate):
    def __init__(self, model_cfg, num_point_features, voxel_size, point_cloud_range, **kwargs):
        ...
        Args:
            model_cfg:                  MODEL_VFE配置部分
                ...
                USE_NORM: True
                NUM_FILTERS: [64]
            num_point_features:         原始特征维度，原始的特征维度是4
            voxel_size:                 体素大小
        ...
        self.feature_select_dict = self.model_cfg.FEATURE_SELECT  # 特征编码的选择

        # 输入特征维度的确定
        if self.feature_select_dict.get('USE_OFFSET_INFO', False):      # 质心偏移+网格偏移特征 (6)
            num_point_features += 6
        if self.feature_select_dict.get('USE_POINT_MEAN', False):       # 质心编码 (3)
            num_point_features += 3
        if self.feature_select_dict.get('USE_ABSLOTE_XYZ_SECOND', False):   # 几何信息高阶编码 (3)
            num_point_features += 3
        if self.feature_select_dict.get('USE_OFFSET_NORM', False):      # 几何偏移范数 + 质心偏移范数 (2)
            num_point_features += 2
        if self.feature_select_dict.get('USE_ANGLE_INFO', False):       # 水平方向 + 垂直方向 (2)
            num_point_features += 2
        if self.feature_select_dict.get('USE_DISTANCE_INFO', False):    # 距离信息 + 深度信息 (2)
            num_point_features += 2
    ...
    def forward(self, batch_dict, **kwargs):
        ...
        points_mean = voxel_features[:, :, :3].sum(dim=1, keepdim=True) / voxel_num_points.type_as(voxel_features).view(
            -1, 1, 1)  # 计算每个voxel内有效点的中心坐标(无效点部分用已0表示)
        ...
        # 质心偏移+网格偏移特征 (6)
        if self.feature_select_dict.get('USE_OFFSET_INFO', False):
            features.extend([f_cluster, f_center])

        # 质心编码 (3)
        if self.feature_select_dict.get('USE_POINT_MEAN', False):
            f_point_mean = points_mean.repeat(1, voxel_features.shape[1], 1)  # 对每个点都进行重复来拼接 (N, 32, 1)
            features.extend([f_point_mean])

        # 几何信息高阶编码 (3)
        if self.feature_select_dict.get('USE_ABSLOTE_XYZ_SECOND', False):
            f_xyz_second = voxel_features[:, :, :3] ** 2  # 二阶位置特征 (N, 32, 3)
            features.extend([f_xyz_second])

        # 几何偏移范数 + 质心偏移范数 (2)
        if self.feature_select_dict.get('USE_OFFSET_NORM', False):
        ...
        # 水平方向 + 垂直方向 (2)
        if self.feature_select_dict.get('USE_ANGLE_INFO', False):
            f_depth = torch.norm(voxel_features[:, :, :2], 2, 2, keepdim=True)  # xy的二范数就是深度信息
            # 对于角度的计算需要避免分母为0的情况，会得到nan与inf的结果
            f_horizontal_dir = \
                torch.arctan(voxel_features[:, :, 1:2] / (voxel_features[:, :, :1] + 1e-5)).clamp(min=-1.57, max=1.57)   # argtan(y/x)
            f_vertical_dir = \
        ...
        # 距离信息 + 深度信息 (2)
        if self.feature_select_dict.get('USE_DISTANCE_INFO', False):
