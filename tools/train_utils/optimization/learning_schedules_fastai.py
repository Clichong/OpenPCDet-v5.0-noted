...
class LRSchedulerStep(object):
    def __init__(self, fai_optimizer: OptimWrapper, total_step, lr_phases,
        ...
        self.optimizer = fai_optimizer  # Adam
        self.total_step = total_step    # 4640
        self.lr_phases = []
        ...
        #  self.lr_phases =
        # [(0, 1856, functools.partial(<function annealing_cos at 0x7f77793c97a0>, 0.00030000000000000003, 0.003)),
        # (1856, 4640, functools.partial(<function annealing_cos at 0x7f77793c97a0>, 0.003, 3.0000000000000004e-08))]
        ...
        #  self.mom_phases =
        # [(0, 1856, functools.partial(<function annealing_cos at 0x7fc998fc97a0>, 0.95, 0.85)),
        # (1856, 4640, functools.partial(<function annealing_cos at 0x7fc998fc97a0>, 0.85, 0.95))]

    # 功能: 这里的step是指迭代次数，根据迭代次数更新学习率: lr_scheduler.step(accumulated_iter)
    def step(self, step):
...
class OneCycle(LRSchedulerStep):
    def __init__(self, fai_optimizer, total_step, lr_max, moms, div_factor,
        ...
        Args:
            fai_optimizer:  Adam
            total_step: 总迭代次数: epoch * len(dataloader)
            lr_max:     学习率0.003
            moms:       动量[0.95, 0.85]
            div_factor: 10
            pct_start:  0.4
        ...
        self.div_factor = div_factor    # 10
        self.pct_start = pct_start      # 0.4
        a1 = int(total_step * self.pct_start)   # 1856
        a2 = total_step - a1
        low_lr = self.lr_max / self.div_factor  # 0.0003

        # 两个部分的start和end相反,代用annealing_cos生成趋势上升和下降的曲线，且限制在start和end之间
        # 在OneCycle策略中，伴随着先大学习率转变时，降低动量会带来更好的效果，所以mom和lr的变化是相反的
        lr_phases = ((0, partial(annealing_cos, low_lr, self.lr_max)),      # (0.0003, 0.003)
                     (self.pct_start,
                      partial(annealing_cos, self.lr_max, low_lr / 1e4)))   # (0.003, 3e-8)
        mom_phases = ((0, partial(annealing_cos, *self.moms)),      # (0.95, 0.85)
                      (self.pct_start, partial(annealing_cos,
                                               *self.moms[::-1])))  # (0.85, 0.95)
        fai_optimizer.lr, fai_optimizer.mom = low_lr, self.moms[0]
        super().__init__(fai_optimizer, total_step, lr_phases, mom_phases)   # adam, 4640, lr_phases， mom_phases
