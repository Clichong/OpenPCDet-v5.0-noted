...
# 功能: 将model中的全部bn层单独构建为一个nn.Sequential，其他的再构建成一个nn.Sequential
def split_bn_bias(layer_groups):
    ...
    for l in layer_groups:
        l1, l2 = [], []
        for c in l.children():
            if isinstance(c, bn_types):   # 如果是bn层存放在l2列表
                l2.append(c)
            else:                         # 非bn层存放在l1列表
                l1.append(c)
...
def listify(p=None, q=None):
    ...
    if p is None:
        ...
        p = [p]     # [0.003]
    n = q if type(q) == int else len(p) if q is None else len(q)    # 1
    if len(p) == 1: p = p * n
    assert len(p) == n, f'List len mismatch ({len(p)} vs {n})'
    return list(p)

# 功能: 保留序列中可导部分, 其他则进行过滤
def trainable_params(m: nn.Module):
    "Return list of trainable params in `m`."
    res = filter(lambda p: p.requires_grad, m.parameters())    # 保留requires_grad=True的模块
    return res
...
class OptimWrapper():
    ...
    def create(cls, opt_func, lr,
        ...
        """
        Args:
            opt_func:   初始化的adam优化器
            lr:         学习率
            layer_groups:   model网络层序列
            **kwargs:       包含wd=optim_cfg.WEIGHT_DECAY, true_wd=True, bn_wd=True
        """
        "Create an `optim.Optimizer` from `opt_func` with `lr`. Set lr on `layer_groups`."
        split_groups = split_bn_bias(layer_groups)  # 构建两个nn.Sequential,其中bn层单独一个nn.Sequential
        opt = opt_func([{'params': trainable_params(l), 'lr': 0} for l in split_groups])    # Adam两组参数,学习率初始化为0

        # cls 是构造函数，它将构造类并调用 __init__(self,) 函数
        # 相当于: opt = OptimWrapper(opt, **kwargs)
        opt = cls(opt, **kwargs)
